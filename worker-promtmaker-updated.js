...
// Version: v20250128-rev3 (Added prompt-stage support for staged generation)
...
// NEW: Validate prompt-stage payload (single stage content)
function validatePromptStagePayload(obj) {
  const issues = [];
  const p = obj?.prompts;
...
// NEW: Build repair prompt for prompt-stage (single stage)
function buildRepairPromptStage(issues, originalJSON) {
  let repairInstructions = [
    "You returned JSON that failed validation.",
    "Fix ONLY the listed issues. Keep structure and IDs stable.",
    "Return ONLY valid JSON (no markdown, no code fences).",
    ...
    "",
    "═══════════════════════════════════════════════════════════════",
    "CRITICAL: The stage content is too short or incomplete.",
    "═══════════════════════════════════════════════════════════════",
    "",
    "You MUST expand the fullPrompt to include:",
    "",
...
async function retryWithRepair(env, msgs, stage, attachMetaAndValidate) {
  ...
  // Use different repair prompt builder for prompt-stage
  const repairPrompt = stage === "prompt-stage" 
    ? buildRepairPromptStage(issues, JSON.stringify(out || {}))
    : buildRepairPrompt(issues, JSON.stringify(out || {}));
  ...
  try {
    ...
    const attachMetaAndValidate = (obj, expectedStage) => {
      ...
      switch (expectedStage) {
        ...
        case "prompt-stage": {
          // NEW: Support for single stage generation
          const v = validatePromptStagePayload(obj);
          return v.ok ? { ok: true, value: { ...obj, correlationId } } : { ok: false, issues: v.issues };
        }
        case "prompts": {
