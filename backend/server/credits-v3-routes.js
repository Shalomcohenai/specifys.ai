 ...
 * Returns: { unlimited, total, breakdown, subscription, permissions }
 */
...
router.post('/consume', verifyFirebaseToken, async (req, res, next) => {
  ...
  }, '[credits-v3-routes] POST /consume - Request received');
  
  try {
    if (!userId) {
      logger.error({ requestId }, '[credits-v3-routes] No userId in request');
    ...
    if (!specId || typeof specId !== 'string') {
      logger.warn({ requestId, userId, specId, specIdType: typeof specId }, '[credits-v3-routes] Invalid specId');
      return next(createError('specId is required and must be a string', ERROR_CODES.MISSING_REQUIRED_FIELD, 400));
    }

    logger.debug({ requestId, userId, specId, priority }, '[credits-v3-routes] Calling consumeCredit service');
    
    const result = await creditsV3Service.consumeCredit(userId, specId, { priority });
    ...
    }, '[credits-v3-routes] POST /consume - Success');
    ...
    }, '[credits-v3-routes] ❌ POST /consume - Error occurred');
...
router.post('/grant', requireAdmin, async (req, res, next) => {
  ...
  logger.info({ requestId }, '[credits-v3-routes] POST /grant - Granting credits');
  ...
  try {
    ...
    logger.info({ requestId, userId, amount }, '[credits-v3-routes] POST /grant - Success');
    res.json({
      success: true,
      ...result
    });
  } catch (error) {
    logger.error({ requestId, error: { message: error.message, stack: error.stack } }, '[credits-v3-routes] POST /grant - Error');
...
router.post('/refund', verifyFirebaseToken, async (req, res, next) => {
  ...
  logger.info({ requestId, userId: req.user?.uid }, '[credits-v3-routes] POST /refund - Refunding credits');
  ...
  try {
    ...
    if (!isAdmin && targetUserId !== requestingUserId) {
      logger.warn({ requestId, requestingUserId, targetUserId }, '[credits-v3-routes] Forbidden: User can only refund own credits');
    ...
    if (!reason || typeof reason !== 'string') {
      logger.warn({ requestId }, '[credits-v3-routes] Invalid reason');
      return next(createError('reason is required and must be a string', ERROR_CODES.MISSING_REQUIRED_FIELD, 400));
    }

    // Refund credits
    const result = await creditsV3Service.refundCredit(
      targetUserId,
      amount,
      reason,
    ...
    logger.info({ requestId, targetUserId, amount }, '[credits-v3-routes] POST /refund - Success');
    res.json({
      success: true,
      ...result
    });
  } catch (error) {
    logger.error({ requestId, error: { message: error.message, stack: error.stack } }, '[credits-v3-routes] POST /refund - Error');
...
router.get('/ledger', verifyFirebaseToken, async (req, res, next) => {
  ...
  logger.info({ requestId, userId: req.user?.uid }, '[credits-v3-routes] GET /ledger - Fetching ledger');
  ...
  try {
    ...
    if (!isAdmin && targetUserId !== requestingUserId) {
      logger.warn({ requestId, requestingUserId, targetUserId }, '[credits-v3-routes] Forbidden: User can only view own ledger');
      return next(createError('Forbidden', ERROR_CODES.FORBIDDEN, 403, {
    ...
    const result = await creditsV3Service.getCreditLedger(targetUserId, filters);

    logger.info({ requestId, targetUserId, count: result.count }, '[credits-v3-routes] GET /ledger - Success');
    res.json({
      success: true,
      ...result
    });
  } catch (error) {
    logger.error({ requestId, error: { message: error.message, stack: error.stack } }, '[credits-v3-routes] GET /ledger - Error');
...
router.get('/history', verifyFirebaseToken, async (req, res, next) => {
  ...
  try {
    ...
    const consumeLedger = await creditsV3Service.getCreditLedger(userId, { type: 'consume', limit: 1000 });
    
    // Get recent transactions for display (last 10)
    const recentLedger = await creditsV3Service.getCreditLedger(userId, { limit: 10 });
    ...
    res.json({
      success: true,
      summary: summary,
      recentTransactions: recentLedger.transactions
    });
  } catch (error) {
