...
const SPEC_GENERATOR_INSTRUCTIONS =
  'You generate application specification sections (overview, technical, market, design, architecture, visibility, prompts). Return only valid JSON matching the structure requested in the user message. No markdown, no explanation. Priority: (1) USER REQUIREMENTS verbatim — do not omit or contradict user-provided pages, workflows, features, integrations, or design; (2) App Description for narrative; (3) inference only for gaps. List gap-fills in inferredItems metadata only — NEVER put [INFERRED], TODO, TBD, or loading placeholders in user-facing names or descriptions. Keep ideaSummary concise and purposeful; keep glossary and epics tight (no filler). Do not add auth, real-time, AI, or mobile unless the user requested them or the spec explicitly requires them.';
