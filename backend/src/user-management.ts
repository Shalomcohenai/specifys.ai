...
let lastUserSyncReport: any = null;

interface IdListSummary {
  total: number;
  sample: string[];
}

function summarizeIdList(list: string[]): IdListSummary {
  return {
    total: list.length,
 ...
 */
export async function initializeUser(uid: string, userDataOverrides: Record<string, any> = {}): Promise<InitializeUserResult> {
  try {
    ...
    return await db.runTransaction(async (transaction) => {
      ...
      const entitlementsDocToWrite: any = {
        userId: uid,
        unlimited: false,
        can_edit: false,
...
interface EnsureEntitlementResult {
  created: boolean;
  updated: boolean;
  unchanged: boolean;
  data: any;
...
export async function ensureEntitlementDocument(uid: string, overrides: Record<string, any> = {}): Promise<EnsureEntitlementResult> {
  const entitlementsRef = db.collection('entitlements').doc(uid);
  ...
  if (!isNew) {
    ...
    const dataToWrite: any = { ...overrides };
  ...
  const dataToWrite: any = { ...defaultData, ...overrides };
...
interface SyncAllUsersOptions {
  ensureEntitlements?: boolean;
  includeDataCollections?: boolean;
  dryRun?: boolean;
...
interface SyncAllUsersResult {
  ...
  errors: number;
  errorDetails: Array<{ uid: string; email: string | null; message: string }>;
  potentialCreates?: number;
  potentialEntitlementCreates?: number;
  firestoreTotal?: number;
  entitlementsTotal?: number;
  inconsistencies?: {
    authWithoutFirestore: IdListSummary;
    firestoreWithoutAuth: IdListSummary;
    missingEntitlements: IdListSummary;
    dataWithoutAuth?: IdListSummary;
  };
  collectionStats?: {
    specs: number;
    apps: number;
 ...
 */
export async function syncAllUsers(options: SyncAllUsersOptions = {}): Promise<SyncAllUsersResult> {
  const {
  ...
  try {
    ...
    const summary: SyncAllUsersResult = {
      runAt,
      durationMs: 0,
    ...
    const firestoreUserIds = new Set(firestoreUsers.map((user: any) => user.id));
...
export async function getUserStats(): Promise<any> {
  try {
    ...
    const firestoreUserIds = new Set(firestoreUsers.map((u: any) => u.id));
    const dataUserIds = new Set<string>();
...
export function getLastUserSyncReport(): any {
 ...
 */
export async function cleanupOrphanedData(): Promise<{ cleanedSpecs: number; cleanedApps: number; cleanedMarket: number; total: number }> {
  try {
