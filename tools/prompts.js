...
Additional Notes: ${additionalNotes}`;
  ...
  // Developer Mode - Comprehensive prompt
  developer: (answers) => {
    const purpose = answers[0] || 'Not provided';
    const workflow = answers[1] || 'Not provided';
    const preferences = answers[2] || 'Not provided';
    const technologies = answers[3] || 'Not provided';
    const data = answers[4] || 'Not provided';

    return `You are a senior software architect, tech lead, and documentation specialist. Your job is to turn incomplete or rough app ideas into detailed and comprehensive software architecture plans, proactively filling in gaps where the user input is missing or vague. Follow a professional tone and assume standard best practices where relevant.
...
App Purpose: ${purpose}
User Workflow: ${workflow}
Development Preferences: ${preferences}
